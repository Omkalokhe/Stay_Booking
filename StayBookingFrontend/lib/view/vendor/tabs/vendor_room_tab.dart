...
class VendorRoomTab extends StatelessWidget {
        ...
        return SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 12),
              Obx(
                () => c.rooms.isEmpty && !c.isLoading.value
                    ? _emptyState()
                    : _roomGrid(context, c),
              ),
              const SizedBox(height: 12),
  ...
  VendorRoomController _resolveController({required String tag}) {
    ...
    return Get.put(VendorRoomController(user: user), tag: tag, permanent: true);
  }
                        ...
                        Expanded(
                          child: FilledButton(
                            onPressed: () {
                              ...
                              controller.setAvailabilityFilter(
      ...
      builder: (context, constraints) {
        ...
        return Wrap(
          ...
          children: c.rooms
              .map(
                (room) =>
                    SizedBox(width: cardWidth, child: _card(context, c, room)),
              )
              .toList(),
        );
      },
  ...
  Future<void> _openForm(BuildContext context, VendorRoomController c) async {
    ...
    final dialogWidth = screenWidth < 640 ? screenWidth * 0.94 : 560.0;
                ...
                child: Obx(
                  () => Column(
                    ...
                    children: [
                      if (!isEdit) ...[
                        TextFormField(
                          controller: c.hotelIdController,
                          keyboardType: TextInputType.number,
                          validator: c.validateHotelId,
                          decoration: _input(
                            'Hotel ID',
                      ...
                      TextFormField(
                        ...
                        validator: (v) => c.requiredField(v, 'Room type'),
                        decoration: _input(
                          'Room Type',
                      ...
                      TextFormField(
                        ...
                        maxLines: 3,
                        validator: (v) => c.requiredField(v, 'Description'),
                        decoration: _input('Description', Icons.notes_rounded),
                      ),
                      const SizedBox(height: 8),
                      SwitchListTile(
                        contentPadding: EdgeInsets.zero,
                        value: c.available.value,
                        onChanged: (v) => c.available.value = v,
                        title: const Text('Available'),
                      ),
                      if (isEdit)
                        CheckboxListTile(
                          contentPadding: EdgeInsets.zero,
                          value: c.replacePhotos.value,
                          onChanged: (v) => c.replacePhotos.value = v ?? false,
                          title: const Text('Replace existing photos'),
                            ...
                            children: c.selectedPhotos
                                .map(
                                  (file) => InputChip(
                                    ...
                                    onDeleted: () => c.removePickedPhoto(file),
                                  ),
                                )
                                .toList(),
                          ),
