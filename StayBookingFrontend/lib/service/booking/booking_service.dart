...
class BookingService {
  ...
  Future<ServicePagedResult<BookingResponseDto>> getBookings({
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      return ServicePagedResult<BookingResponseDto>(
        success: true,
        message: _parser.extractMessage(decoded) ?? 'Bookings loaded successfully.',
        pageData: pageData,
      );
    }
  ...
  Future<SimpleResult> deleteBooking(int id) async {
    ...
    if (response.statusCode == 204) {
      return const SimpleResult(success: true, message: 'Booking deleted successfully.');
    }
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      return SimpleResult(
        success: true,
        message: _parser.extractMessage(decoded) ?? 'Booking deleted successfully.',
      );
    }
  ...
  HotelActionResult<BookingResponseDto> _toActionResult(
    ...
    return HotelActionResult<BookingResponseDto>(
      success: false,
      message:
          _parser.extractMessage(decoded) ?? 'Request failed (${response.statusCode})',
      item: item,
    );
  }
